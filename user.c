...
//update the statement of the chosen book
			while(borrowed_current->next != NULL)
			...
			borrowedlist->length ++;
			current->currentborroweduser = currentuser;
			
}

void return_a_book(User* currentuser, BookList* booklist, BookList* borrowedlist){
	Book* borrowedlist_current = borrowedlist->list->borrowedlist_next;
      printf("id\ttitle\tauthor\tyear\tcopies\n");
	unsigned int option;
//print the borrowed book of this account
      while(borrowedlist_current->next != NULL){
		if(strcmp(borrowedlist_current->currentborroweduser->username, currentuser->username))
